...
var ObservableQuery = (function (_super) {
    ...
    ObservableQuery.prototype.refetch = function (variables) {
        ...
        if (fetchPolicy === 'cache-only') {
            return Promise.reject(process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(1) : new tsInvariant.InvariantError('cache-only fetchPolicy option should not be used together with query refetch.'));
        }
...
var ApolloClient = (function () {
    function ApolloClient(options) {
        ...
        if (!cache) {
            throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(2) : new tsInvariant.InvariantError("To initialize Apollo Client, you must specify a 'cache' property " +
    ...
    ApolloClient.prototype.query = function (options) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(options.fetchPolicy !== 'cache-and-network', 3) : tsInvariant.invariant(options.fetchPolicy !== 'cache-and-network', 'The cache-and-network fetchPolicy does not work with client.query, because ' +
        ...
        workSet.forEach(function (selection) {
            ...
            if (isField(selection)) {
                ...
                if (typeof value !== 'undefined') {
                    ...
                    throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(52) : new tsInvariant.InvariantError("Missing field '" + resultFieldKey + "' in " + JSON.stringify(result, null, 2).substring(0, 100));
                }
            }
            else {
...
var ApolloProvider = function (_a) {
    ...
    return React.createElement(ApolloContext.Consumer, null, function (context) {
        ...
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(context.client, 9) : tsInvariant.invariant(context.client, 'ApolloProvider was not passed a client instance. Make ' +
...
function parser(document) {
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(!!document && !!document.kind, 5) : tsInvariant.invariant(!!document && !!document.kind, "Argument of " + document + " passed to parser was not a valid GraphQL " +
    ...
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), 6) : tsInvariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), "Passing only a fragment to 'graphql' is not yet supported. " +
        "You must include a query, subscription or mutation as well");
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, 7) : tsInvariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, "react-apollo only supports a query, subscription, or a mutation per HOC. " +
    ...
    var definitions = queries.length
        ...
        : mutations.length
            ? mutations
            : subscriptions;
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(definitions.length === 1, 8) : tsInvariant.invariant(definitions.length === 1, "react-apollo only supports one definition per HOC. " + document + " had " +
